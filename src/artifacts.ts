...
import { MAX_ARTIFACT_TTL_HOURS, type ServerConfig } from "./config.js";
...
function normalizeTtlHours(value: number | undefined, fallback: number): number {
  ...
  if (
    ...
    || ttlHours < 1
    || ttlHours > MAX_ARTIFACT_TTL_HOURS
  ) {
    throw new ArtifactError(
      "invalid_ttl",
      `Artifact TTL must be an integer between 1 and ${MAX_ARTIFACT_TTL_HOURS} hours.`,
    );
  }
  return ttlHours;
}
